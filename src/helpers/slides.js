export function createContentArray(alt, photos) {
  ...
  */
  ...
  if (photosIsAnArray()) {
    contentArray = photos.map((photo, idx) => {
      return content(alt, idx, photo);
    });
  } else if (photosIsAnObject()) {
    contentArray = [content(alt, 0, photos)];
  }

  return contentArray;

  function altIsNotAString() {
    return typeof alt !== 'string';
  }

  function photosIsAnArray() {
    return Array.prototype.isPrototypeOf(photos);
  }

  function photosIsAnObject() {
    return Object.prototype.isPrototypeOf(photos);
  }

  function content(alt, idx, photo) {
    return {
      alt: `${alt} ${idx + 1}`,
      id: photo.id,
